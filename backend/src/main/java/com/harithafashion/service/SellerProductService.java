...
public class SellerProductService {
    ...
    public PageResponse<ProductCardResponse> listMyProducts(UUID userId, int page, int size, ProductStatus status) {
        ...
        return PageResponse.from(p.map(pr -> productService.getProductCard(pr.getId())));
    }
    ...
    @Transactional
    public Product createProduct(UUID userId, CreateProductRequest req) {
        Seller seller = sellerRepository.findByUserId(userId).orElseThrow();
    ...
    @Transactional
    public Product updateProduct(UUID userId, UUID productId, UpdateProductRequest req) {
    ...
    @Transactional
    public Product updateStatus(UUID userId, UUID productId, ProductStatus status) {
        Product product = verifyOwnership(userId, productId);
    ...
    @Transactional
    public ProductImage addImage(UUID userId, UUID productId, ProductImageRequest req) {
        Product product = verifyOwnership(userId, productId);
        int sort = req.getSortOrder() != null ? req.getSortOrder() : imageRepository.findByProductIdOrderBySortOrderAsc(productId).size();
        return addImageInternal(product, req, sort);
    }
    ...
    public Page<Product> adminListProducts(int page, int size, ProductStatus status, UUID sellerId) {
        PageRequest pr = PageRequest.of(page, size);
        if (sellerId != null && status != null) {
            return productRepository.findBySellerIdAndStatus(sellerId, status, pr);
        }
        if (sellerId != null) return productRepository.findBySellerId(sellerId, pr);
        if (status != null) return productRepository.findByStatus(status, pr);
    ...
    private ProductImage addImageInternal(Product product, ProductImageRequest req, int sort) {
        return imageRepository.save(ProductImage.builder()
                ...
                .isPrimary(req.getIsPrimary() != null ? req.getIsPrimary() : sort == 0)
